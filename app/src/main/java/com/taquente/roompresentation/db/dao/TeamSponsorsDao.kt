...
interface TeamSponsorsDao {
    ...
    @Query("SELECT id, name FROM Teams INNER JOIN Team_Sponsor_Join ON Teams.id = Team_Sponsor_Join.teamId WHERE Team_Sponsor_Join.sponsorId =:sponsorId")
    fun getTeamsForSponsor(sponsorId: Int): List<Team>

    @Query("SELECT id, name FROM Sponsors INNER JOIN Team_Sponsor_Join ON Sponsors.id = Team_Sponsor_Join.sponsorId WHERE Team_Sponsor_Join.teamId =:teamId")
